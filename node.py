...
class Node():
    ...
    def commit(self):
        ...
        query_type = self.staged["query_type"]
        query_key = self.staged["query_key"]
        response = None
        if self.status == LEADER :
            if query_type == "cart" and query_key == "create_cart" :
                cart_dao = CartDAO()
                response = cart_dao.create_cart(self.staged["buyer_id"])
            if query_type == "cart" and query_key == "get_cart" :
                cart_dao = CartDAO()
                response = cart_dao.get_cart(self.staged["buyer_id"])
